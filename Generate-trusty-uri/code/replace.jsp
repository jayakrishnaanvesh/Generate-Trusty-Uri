...
<div class="col-sm-2 sidenav">
  ...
  <h1><a href="index.html">Making Digital<span> Artifacts</span></a></h1>
	...
	Class.forName("com.mysql.jdbc.Driver");
      	...
      	try 
	{
            PreparedStatement p=connection.prepareStatement("select * from ownerfile where fname=? and owner=?"); 
            p.setString(1,s1);
            p.setString(2,s3);
			ResultSet rs=p.executeQuery();
			while(rs.next()){
			String cont=rs.getString("ct");	
			// String cont=c.substring(1, c.length());
			PrintStream pr = new PrintStream(new FileOutputStream("C:/Tomcat 9.0/webapps/pro/files/"+s1));
			pr.print(new String(cont+"\n"));
			PreparedStatement pstmt =connection.prepareStatement("update atkfile set fstat=? where fname=? and oname=? ");
			...
			pstmt.executeUpdate();
			 ...
			 %>
	  <h3>File updated successfully</h3>
	  <%
			}
			connection.close();
			}
